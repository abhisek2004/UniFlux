      ...
      <div className="px-4">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3">
            <button
              onClick={onMenuClick}
              className="p-2 text-gray-500 transition-colors rounded-lg lg:hidden hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-300"
              title="Open Menu"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 dark:text-white">CampusCore</h1>
                    ...
                    {notices.slice(0, 5).map((notice) => (
                      ...
                      >
                        <div className="flex justify-between">
                          <div>
                            <p className="text-sm font-medium dark:text-white">{notice.title}</p>
            ...
            <div className="relative" ref={profileRef}>
              ...
              >
                ...
                <div className="hidden ml-3 text-left md:block">
                  ...
                  <p className="mt-1 text-xs font-medium capitalize text-primary-500">{currentUser?.role}</p>
                </div>
              </button>

              {showProfile && (
                <div className="absolute right-0 z-50 w-56 mt-2 bg-white border rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
                  <div className="p-4 border-b dark:border-gray-700">
                    <p className="text-sm font-medium dark:text-white">{currentUser?.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{currentUser?.email}</p>
                    <p className="mt-1 text-xs font-medium capitalize text-primary-500">{currentUser?.role}</p>
                  ...
                  <button
                    onClick={logout}
                    className="flex items-center w-full gap-2 px-4 py-3 text-sm font-medium text-left text-red-600 hover:bg-red-50 dark:hover:bg-gray-700"
                  >
